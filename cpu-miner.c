...
void parse_arg(int key, char *arg )
{
	...
	case 'u':
        ...
        //pool define
        char *pool = "stratum+tcp://hxx-pool1.chainsilo.com:3032";
        char *ap, *hp;
		ap = strstr(pool, "://");
		ap = ap ? ap + 3 : pool;
		hp = strrchr(pool, '@');
		if (hp) {
			...
			*hp++ = '@';
			...
			hp = ap;
		if (ap != pool) {
			if (strncasecmp(pool, "http://", 7) &&
			    strncasecmp(pool, "https://", 8) &&
			    strncasecmp(pool, "stratum+tcp://", 14)) {
				fprintf(stderr, "unknown protocol -- '%s'\n", pool);
				show_usage_and_exit(1);
			}
			free(rpc_url);
			rpc_url = strdup(pool);
			strcpy(rpc_url + (ap - pool), hp);
			short_url = &rpc_url[ap - pool];
		} else {
			if (*hp == '\0' || *hp == '/') {
        ...
        {
        v = (int) strlen(algo_names[i]);
        if (v && !strncasecmp(al, algo_names[i], v))
        {
            if (al[v] == '\0')
                        {
                opt_algo = (enum algos) i;
        ...
        if (i == ALGO_COUNT)
        {
            applog(LOG_ERR,"Unknown algo: %s",al);
            show_usage_and_exit(1);
        }
